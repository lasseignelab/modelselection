...
saveRDS(cl_tumtis_indgene, "../data/recount3/corres/cl_tumtis_indgene.rds")

cl_ndtis_indgene <- cor_eachgene(cellline_tpm, con_tis_tpm)
saveRDS(cl_ndtis_indgene, "../data/recount3/corres/cl_ndtis_indgene.rds")

pdx_tumtis_indgene <- cor_eachgene(pdx_tpm, tumor_tis_tpm)
saveRDS(pdx_tumtis_indgene, "../data/recount3/corres/pdx_tumtis_indgene.rds")

pdx_ndtis_indgene <- cor_eachgene(pdx_tpm, con_tis_tpm)
saveRDS(pdx_ndtis_indgene, "../data/recount3/corres/pdx_ndtis_indgene.rds")
...
#print stats example
print(paste("Neg Rho, Sig P =", length(rownames(subset(cl_tumtis_indgene, rho < 0) %>% subset(p.value < 0.05)))))
print(paste("Neg Rho, Not Sig P =", length(rownames(subset(cl_tumtis_indgene, rho < 0) %>% subset(p.value >= 0.05)))))
print(paste("Pos Rho, Sig P =", length(rownames(subset(cl_tumtis_indgene, rho > 0) %>% subset(p.value < 0.05)))))
print(paste("Pos Rho, Not Sig P =", length(rownames(subset(cl_tumtis_indgene, rho > 0) %>% subset(p.value >= 0.05)))))
```
...
* CL vs Tum Tissue: 2005
* CL vs Con Tissue: 2151
* PDX vs Tum Tissue: 2319
...
When correlated with tumor tissue:
  * exclusive to cell lines: 1894
* exclusive to pdx: 2208
* in both: 11
...
for(i in c("cl_tumtis", "cl_ndtis", "pdx_tumtis", "pdx_ndtis")){
  ...
  for(i in c("sig", "insig")){
    ...
    temp <- ggplot(indgene_cor_pathway_res_sub_dfs_posneg[[comparison]][[i]], aes(x=reorder(term_name, neg_log10p), y=neg_log10p, color= posneg)) + geom_point(aes(size = recall)) + coord_flip() + xlab("Pathway") + ylab("-log10p") + ggtitle(paste("Pathway Enrichment for", title[which(c("cl_tumtis", "cl_ndtis", "pdx_tumtis", "pdx_ndtis") == comparison)])) + labs(color = "Correlation", size = "Recall") + theme_bw() + theme(text = element_text(size=8, face = "bold"), axis.text.y = element_text(size = 8, face = "bold"), legend.text = element_text(size = 9, face = "bold"), legend.title = element_text(size = 10, face = "bold"), axis.title = element_text(size = 10, face = "bold"), title = element_text(size = 8, face = "bold")) + scale_size(range = c(0.5,3)) + guides(colour = guide_legend(override.aes = list(size=3)))
    
    ggexport(temp, filename = paste("../plots/ind_gene/dotplot/final/", comparison, "_", i, ".png", sep = ""), width = 3000, height = 1800, res = 600)
  }
}
```
