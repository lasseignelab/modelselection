...
gtex_up_tissue_genes <- qusage::read.gmt("/Users/avery/Desktop/work/R Projects/modelselection/data/gene_set_library_up_crisp.gmt")
gtex_down_tissue_genes <- qusage::read.gmt("/Users/avery/Desktop/work/R Projects/modelselection/data/gene_set_library_dn_crisp.gmt")
```
...
source("/Users/avery/Desktop/work/R Projects/modelselection/scripts/functions/tisgenesetcor.R")
```
...
for(i in tpm_to_pull){
  assign(paste(i, "_tpm", sep = ""), readRDS(paste("~/Desktop/work/R Projects/modelselection/data/recount3/tpm/", i, "_tpm.rds", sep = "")))
}
...
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/allcltomatchtum.png", 
          width = 13, height = 7, units = "in")

ggplot(tis_gene_cor_con_vio_in, aes(x=tissue, y=corres, color=tissue)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + labs(x = "Matched Control Tissue", y = "Median Correlation of Each Cell Line Sample to Matched Control Tissue") + ylim(0.4,0.9)
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/allcltomatchcon.png", 
          width = 13, height = 7, units = "in")
```
...
```{r}
source("/Users/avery/Desktop/work/R Projects/modelselection/scripts/functions/geom_split_violin.R")
```
...
tis_gene_cor_both_vio_in$Type <- c(rep("Tumor", length(tis_gene_cor_tum_vio_in$corres)), rep("Control", length(tis_gene_cor_con_vio_in$corres)))
...
ggplot(tis_gene_cor_both_vio_in, aes(tissue, corres, fill = Type)) + geom_split_violin(width = 1) + labs(x = "Tissue of Origin", y = "Median Correlation Between Matched Cell Lines and Tissues") + theme(axis.text.x = element_text(angle = 45, hjust=1)) + scale_fill_manual(values = c("darkblue", "orange")) + labs(fill = "Tissue Type")
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/allcltomatchboth.png", 
          width = 13, height = 7, units = "in")
```
...
for(i in names(tis_gene_cor_stat)){
  ...
  tis_gene_cor_stat[[i]][["effect"]] <- tis_gene_cor_stat[[i]][["corres"]] %>%
    ...
    add_significance()
  stat.test_tag # print output in window
  # effect size
...
for(i in c("tum", "con")){
  ...
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + ggtitle(paste("Median Correlation of Cell Lines of Each Tissue Type to ", stringr::str_to_title(i), " Tumor Tissue by ", stringr::str_to_title(i), " TAGs", sep = ""))
      ggsave(filename = paste("~/Desktop/work/R Projects/modelselection/plots/tags_violin/", i, "tum.png", sep = ""), 
  ...
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + ggtitle(paste("Median Correlation of Cell Lines of Each Tissue Type to ", stringr::str_to_title(i), " Control Tissue by ", stringr::str_to_title(i), " TAGs", sep = ""))
      ggsave(filename = paste("~/Desktop/work/R Projects/modelselection/plots/tags_violin/", i, "con.png", sep = ""), 
          width = 15, height = 7, units = "in")
    }
  }
}
...
```{r}
# make TPM obj for just brain CLs
brain_cl_tpm <- cl_tpm[,colnames(cl_tpm) %in% sample_master_meta[sample_master_meta$tissue_coerced == "Brain",]$rse_sampname]

# run function
# computes performance of brain cls vs brain pdxs in correlating to brain tissue in brain TAGs
corbytis_pdxcl_tum_res <- tisgenesetcor(cbind(brain_cl_tpm, pdx_tpm), tum_tissue_tpm, tissue = "brain")

corbytis_pdxcl_con_res <- tisgenesetcor(cbind(brain_cl_tpm, pdx_tpm), con_tissue_tpm, tissue = "brain")
```

### visualize as violin plot
#### first, need to make the data length-wise
```{r prep data for violin plot}
originrep <- c(rep("CL", ncol(brain_cl_tpm)), rep("PDX", ncol(pdx_tpm)))
...
```{r generate violin plot}
library(ggplot2)
tis_gene_cor_pdxcl_tum_v_plot <- ggplot(tis_gene_cor_pdxcl_tum_vio_in, aes(x=type, y=corres, color=type)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + labs(x = "Model Type", y = "Median Correlation of Brain Model to Brain Tumor Tissue in Brain TAGs") + ylim(0.5,0.9) + guides(color = "none")

tis_gene_cor_pdxcl_con_v_plot <- ggplot(tis_gene_cor_pdxcl_con_vio_in, aes(x=type, y=corres, color=type)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + labs(x = "Model Type", y = "Median Correlation of Brain Model to Brain Control Tissue in Brain TAGs") + ylim(0.5,0.9) + guides(color = "none")
```
...
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/brainclpdxtomatchtum.png", 
          width = 5, height = 6)
...
tis_gene_cor_pdxcl_con_v_plot + 
  stat_pvalue_manual(stat.test_tag, tip.length = 0) +
  labs(subtitle = get_test_label(stat.test_tag, detailed = TRUE))
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/brainclpdxtomatchcon.png", 
          width = 5, height = 6)
```
...
```{r}
library(ggplot2)
ggplot(tis_gene_cor_pdxcl_alltum_vio_in, aes(tissue, cor, fill = type)) + geom_split_violin() + labs(x = "Tumor Tissue Type", y = "Median Correlation of Each Brain Model to Each Tissue Type") + theme(axis.text.x = element_text(angle = 45, hjust=1)) + scale_fill_discrete(name = "Model")
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/brainclpdxtoalltum.png", 
          width = 5000, height = 2000, units = "px")

ggplot(tis_gene_cor_pdxcl_allcon_vio_in, aes(tissue, cor, fill = type)) + geom_split_violin() + labs(x = "Control Tissue Type", y = "Median Correlation of Each Brain Model to Each Tissue Type") + theme(axis.text.x = element_text(angle = 45, hjust=1)) + scale_fill_discrete(name = "Model")
ggsave(filename = "~/Desktop/work/R Projects/modelselection/plots/tags_violin/brainclpdxtoallcon.png", 
          width = 13, height = 7)
```
...
colnames(corbytis_alltis_allcl_df) <- c(paste(stringr::str_to_title(colnames(corbytis_tum_allcl_df)), "Tumor"), paste("Control", stringr::str_to_title(colnames(corbytis_con_allcl_df))))
...
alltis_annocol <- data.frame("Tissue Type" = c(rep("Tumor", length(colnames(corbytis_tum_allcl_df))), rep("Control", length(colnames(corbytis_con_allcl_df)))))
...
# heatmap!
ann_colors <- list("Tissue Type" = c(Control = "darkblue", Tumor = "orange")) # make tissue type anno color consistent
pheatmap(t(corbytis_alltis_100cl_df), annotation_col = alltis_annorow, labels_col = alltis_top100cl_names, fontsize_col = 6, annotation_row = alltis_annocol, annotation_colors = ann_colors)
