...
## by just origin type
colorset <- rainbow(4)
png("../plots/LW_pca_origintype.png" )
...
png("../plots/LW_fullcor_brain_heatmap.png" )
...
## hm time
png("../plots/LW_hm_brainsubs.png", width = 900, height = 1100)
pheatmap(hm_rm, annotation_row = annotation_col, show_rownames = F, angle_col = 45, fontsize = 23, labels_col = c("GBM Tumor Tissue", "All Brain Tumor Tissue", "Brain Control Tissue"), color = colorRampPalette(c("yellow", "red"))(200), annotation_colors = ann_colors)
dev.off()
```
...
ggplot(cl_pdx_violin, aes(x=type, y=con_med_cor, color = type)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + guides(color = "none") + labs(x = "Model Type", y = "Median Correlation of Model to Brain Control Tissue") + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 14), axis.text = element_text(size = 15))
ggsave(filename = "../plots/tags_violin/LW_fullgenesvio_brainclvspdx_con.png", width = 5, height = 6)

con_violin_alt <- ggplot(cl_pdx_violin, aes(x=type, y=con_med_cor, color = type)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + guides(color = "none") + labs(x = "Model Type", y = "Median Correlation of Model Sample to Brain Control Tissue") + ylim(0.5,0.9) + guides(color = "none") # alt with scale difference for fig 3
...
ggsave(filename = "../plots/tags_violin/LW_fullgenesvio_brainclvspdx_tum.png", width = 5, height = 6)
...
#tum_violin + #####Error: object 'tum_violin' not found
...
# report
stat.test <- stat.test %>% add_xy_position(x = "group")
#con_violin +  ####Error: object 'con_violin' not found
#  stat_pvalue_manual(stat.test, tip.length = 0) +
#  labs(subtitle = get_test_label(stat.test, detailed = TRUE))
...
ggsave(filename = "../plots/LW_vio_braincltoeachtum.png", 
          width = 18, height = 7)

ggplot(cor_dismatch_contis_long, aes(tissue, cor, fill = Model)) + geom_split_violin() + labs(x = "Control Tissue Type", y = "Median Correlation of Each Brain-Derived Model to Control Tissue") + theme(axis.text.x = element_text(angle = 45, hjust=1)) + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 12.5), axis.text = element_text(size = 15))
ggsave(filename = "../plots/LW_vio_braincltoeachcon.png", 
          width = 18, height = 7)
```
