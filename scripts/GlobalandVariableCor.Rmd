...
saveRDS(all_pca, "../data/pca_brain.rds")
...
## by just origin type
colorset <- rainbow(4)
png("../plots/GBM_pca_origintype.png")
Scatter_Density(object = all_pca, batch = brain_fact_origin, color.set = colorset, batch.legend.title = "Origin Type", legend.cex = 1.5, legend.title.cex = 1.6, title.cex = 1.6)
...
## hm time
png("../plots/JUSTGBM_hm_brainsubs.png", width = 900, height = 1100)
pheatmap(hm_rm, annotation_row = annotation_col, show_rownames = F, angle_col = 45, fontsize = 23, labels_col = c("GBM Tumor Tissue", "All Brain Tumor Tissue", "Brain Non-Disease Tissue"), color = colorRampPalette(c("yellow", "red"))(200), annotation_colors = ann_colors)
dev.off()
```
...
# Basic violin plot
ggplot(cl_pdx_violin, aes(x=type, y=con_med_cor, color = type)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + guides(color = "none") + labs(x = "Model Type", y = "Median Correlation of Model to Brain Non-Disease Tissue") + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 14), axis.text = element_text(size = 15))
...
ggplot(cl_pdx_violin, aes(x=type, y=tum_med_cor, color = type)) + 
  geom_violin() + geom_jitter(shape=16, position=position_jitter(0.2)) + guides(color = "none") + labs(x = "Model Type", y = "Median Correlation of Model to Brain Tumor Tissue") + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 14), axis.text = element_text(size = 15))
...
```{r plot}
ggplot(cor_dismatch_cantis_long, aes(tissue, cor, fill = Model)) + geom_split_violin() + labs(x = "Tumor Tissue Type", y = "Median Correlation of Each Brain-Derived Model to Tumor Tissue") + theme(axis.text.x = element_text(angle = 45, hjust=1)) + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 12.5), axis.text = element_text(size = 15))
...
ggplot(cor_dismatch_contis_long, aes(tissue, cor, fill = Model)) + geom_split_violin() + labs(x = "Non-Disease Tissue Type", y = "Median Correlation of Each Brain-Derived Model to Non-Disease Tissue") + theme(axis.text.x = element_text(angle = 45, hjust=1)) + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 12.5), axis.text = element_text(size = 15))
...
for(i in c("can", "con")){
    type <- i
    for(i in names(cor_wilcox_matches_alltissues[[paste(type)]])){
        cor_wilcox_matches_alltissues[[type]][[i]][["bon_p"]] <- p.adjust(cor_wilcox_matches_alltissues[[type]][[i]][["p"]], n = length(names(cor_wilcox_matches_alltissues[[type]])), method = "bonferroni")
        cor_wilcox_matches_alltissues[[type]][[i]][["bon_p.signif"]] <- NA
        if(cor_wilcox_matches_alltissues[[type]][[i]][["bon_p"]] < 0.05){
            cor_wilcox_matches_alltissues[[type]][[i]][["bon_p.signif"]] <- "SIG"
        }else{
            cor_wilcox_matches_alltissues[[type]][[i]][["bon_p.signif"]] <- "INSIG"
        }
    }
}

saveRDS(cor_dismatch_stattest_alltis, "../data/cor_wilcox_matches_alltissues.rds")

# how many tissues were significant?
sig_can <- c()
for(i in names(cor_wilcox_matches_alltissues[["can"]])){
    if(cor_wilcox_matches_alltissues[["can"]][[i]][["bon_p.signif"]] == "SIG"){
          sig_can <- c(sig_can, i)
    }
}
print(paste(length(sig_can), "/", length(names(cor_wilcox_matches_alltissues[["can"]]))))

sig_con <- c()
for(i in names(cor_wilcox_matches_alltissues[["con"]])){
    if(cor_wilcox_matches_alltissues[["con"]][[i]][["bon_p.signif"]] == "SIG"){
          sig_con <- c(sig_con, i)
    }
}
print(paste(length(sig_con), "/", length(names(cor_wilcox_matches_alltissues[["con"]]))))
```
