...
#add to docker
library(ggplot2)
library(ComplexHeatmap) #no package called ‘ComplexHeatmap’
library(matrixStats)
```
...
library(ggplot2)
ggplot(corr_table, aes(x = genes_included, y = percent_map_to_origin, color = Variation, linetype = Purity)) + geom_line() + geom_point() + geom_hline(yintercept=percent_map_to_origin[17], color="darkslategray", linetype="dashed") + labs(x = "Number of Genes Included", y = "% of Cell Lines That Correlate Highest to Origin Tissue") + theme(text = element_text(face = "bold"), legend.text = element_text(size = 16), legend.title = element_text(size = 18), axis.title = element_text(size = 12), axis.text = element_text(size = 13))
...
subset_medians <- c()
subsets <- c()
for(i in c(100, 1000, 5000, 10000)){ ###ERROR argument "savefilepath" is missing, with no defaultargume ##double checknt "savefilepath" is missing, with no default
  num <- i
  for(i in c("purinc", "purexc")){
    pur <- i
    for(i in c("tcga", "gtex")){
...
# plot
library(ggplot2)
ggplot(subset_table_lp, aes(x = gene_set_size, y = median_cor_across_tissues, color = Variation, linetype = Purity)) + geom_line() + geom_point() + labs(x = "Number of Genes Included", y = "Median Correlation of Cell Lines to Their Origin Tissue") + geom_hline(aes(yintercept=subset_medians[17]), color="darkslategray", linetype="dashed") ##ERROR 
...
ggsave("../plots/LW_medcortoorigin_tissplit_incpur.png")
...
ggsave("../plots/LW_medcortoorigin_tissplit_excpur.png")
```
