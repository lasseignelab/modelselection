...
```{r}
tcga_tpm  <- readr::read_delim("data/recount3/tpm/tcga_tpm.csv")
tcga_tpm <- tibble::column_to_rownames(tcga_tpm, var = "...1")
gtex_tpm  <- readr::read_delim("data/recount3/tpm/gtex_tpm.csv")
gtex_tpm <- tibble::column_to_rownames(gtex_tpm, var = "...1")
```
...
for(i in project){
  ...
  for(i in subset_num){
    ...
    for(i in var_source){
      ...
      rse_to_subset(rse_sub, project_sub, subset_sub, var_sub, TRUE, TRUE, TRUE, "data/") # with purity
      rse_to_subset(rse_sub, project_sub, subset_sub, var_sub, FALSE, TRUE, TRUE, "data/") # without
    }
  }
}
endTime <- Sys.time()
